...
import {
  ...
  useMediaQuery, useTheme, Divider, Tooltip
} from "@mui/material";
  ...
  useEffect(() => {
    if (brands.length > 0) {
      ...
      brands.forEach((brand) => {
        const domesticLocations = brand?.expansionLocationData?.expansionLocations?.domestic?.locations || [];
        domesticLocations.forEach(loc => loc.state && statesSet.add(loc.state));
      });
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 4, flexWrap: "wrap", gap: 2 }}>
         <Typography variant={isMobile ? "h6" : "h5"} fontWeight="bold" sx={{ color: "#f57c00" }}>
      ...
      {filteredBrands.length === 0 && (
        <Box sx={{ p: 4, textAlign: "center" }}>
          <Typography>No franchise opportunities found in {selectedState}.</Typography>
        </Box>
      )}
