...
const BrandExpansionLocationDetails = ({ data, onChange, errors }) => {
  ...
  (type) => {
    ...
    const totalCities = selections.selectedDistricts.reduce((total, { state, district }) => {
      ...
      // If no cities, count the district as one "city"
      return total + (cities.length > 0 ? cities.length : 1);
    }, 0);
              ...
              onChange={() => {
                selections.selectedDistricts.forEach(({ state, district }) => {
                  ...
                  const cities = stateData?.cities
                    ...
                    .map((c) => c.name) || [];
                  // If no cities, use the district name as the "city"
                  const options = cities.length > 0 ? cities : [district];
          ...
          <Box sx={{ flex: 1, overflow: "auto" }}>
            {Object.entries(districtsByState).map(([stateName, districts]) => {
              ...
              return districts.map((districtName) => {
                ...
                const districtCities = state.cities?.filter((c) => c.district === districtName) || [];
                
                // If no cities, use district name as the only "city"
                const cities = districtCities.length > 0 
                  ? districtCities
                      .map((c) => c.name)
                      .filter((c) => c.toLowerCase().includes(searchFilters.cities.toLowerCase()))
                      .sort()
                  : [districtName];
