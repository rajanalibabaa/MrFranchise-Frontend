...
const CARD_DIMENSIONS = {
  ...
  tablet: { width: 320, height: 420 },
  desktop: { width: 327, height: 400 },
};
...
const BrandCard = React.memo(({ 
  ...
  const brandDetails = brand.brandDetails || {};
  const { brandName = "N/A" } = brandDetails;
        ...
        <Box sx={{ display: "flex", flexDirection: "column", flexGrow: 1 }}>
          <CardContent sx={{ pb: 1 }}>
            ...
            >
              ...
              <IconButton
                onClick={() => handleLikeClick(brand._id, true)}
                disabled={likeProcessing[brand._id]}
                sx={{ ml: 1 }}
              >
                {likeProcessing[brand._id] ? (
                  <CircularProgress size={24} />
                ) : (
...
const LikedBrands = () => {
    ...
    const fetchLikedBrands = async () => {
      try {
        ...
        const response = await axios.get(
          "https://franchise-backend-wgp6.onrender.com/api/v1/like/get-favbrands",
          {
            headers: {
        ...
        setLikedBrands(response.data.data || []);
      } catch (err) {
  ...
  const handleLikeClick = useCallback(async (brandId) => {
    ...
    setLikeProcessing(prev => ({ ...prev, [brandId]: true }));
    try {
      // Call API to unlike the brand
      await axios.delete(
        `https://franchise-backend-wgp6.onrender.com/api/v1/like/remove-favbrand/${brandId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      // Update local state
      setLikedBrands(prev => prev.filter(brand => brand._id !== brandId));
    } catch (error) {
  ...
  return (
    <>
      {likedBrands.length > 0 ? (
        ...
        <Box sx={{ textAlign: "center", p: 4 }}>
          <Typography>No liked brands found</Typography>
        </Box>
      )}
    </>
  );
