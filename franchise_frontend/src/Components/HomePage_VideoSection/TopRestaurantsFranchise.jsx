...
const TopRestaurantsFranchise = () => {
  ...
  const loopingBrands = useMemo(() => {
    return [...beverageBrands, ...beverageBrands.slice(0, 4)];
  }, [beverageBrands]);
  ...
  // Commented out mouse enter/leave handlers for auto-scroll
  ...
  const checkForLoop = useCallback(() => {
    ...
    if (container.scrollLeft >= maxScrollLeft - 100) {
      const originalBrandsCount = beverageBrands.length; // Original brands without duplicates
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Commented out auto-scroll cleanup
      // if (scrollIntervalRef.current) {
      //   clearInterval(scrollIntervalRef.current);
      // }
      if (scrollRequestRef.current) {
  ...
  }, [loopingBrands.length, handleScroll]); // Removed startAutoScroll from dependencies
        ...
        >
          ...
          <Box sx={{ position: "relative", px: isMobile ? 2 : 0 }}>
            ...
            <Box
              ...
              // Removed mouse enter/leave handlers for auto-scroll
