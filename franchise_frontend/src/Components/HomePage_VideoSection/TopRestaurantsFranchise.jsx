...
const TopRestaurantsFranchise = () => {
  ...
  // Filter brands that belong to Multi Cuisine Restaurants subcategory
  const restaurantBrands = useMemo(() => {
    return brands.filter((brand) => {
      const category = brand.franchiseDetails?.brandCategories || {};
      return category.child === "Multi Cuisine Restaurants";
    });
  }, [brands]);
  ...
  }, [restaurantBrands.length, getScrollDistance, smoothScrollTo]);
  ...
  const shouldShow = restaurantBrands.length > 0;
